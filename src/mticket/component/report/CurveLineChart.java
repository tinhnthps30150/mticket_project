...
import mticket.component.transitionForm.FormTransition;
...
public class CurveLineChart extends JComponent {
    ...
    private movieReportJPanel mRP;
    private foodReportJPanel fRP;
    private totalReportJPanel tRP;
    private ModelChart mDC;
    private detailReportMovie dRM;
    private detailReportFood dRF;
    public detailDateReportMovie dDRM = new detailDateReportMovie();
    public detailDateReportFood dDRF = new detailDateReportFood();

    private FormTransition fTST;

    public FormTransition getfTST() {
        return fTST;
    }

    public void setfTST(FormTransition fTST) {
        this.fTST = fTST;
    }

    public movieReportJPanel getmRP() {
        return mRP;
    }

    public void setmRP(movieReportJPanel mRP) {
        this.mRP = mRP;
    }

    public foodReportJPanel getfRP() {
        return fRP;
    }

    public void setfRP(foodReportJPanel fRP) {
        this.fRP = fRP;
    }

    public totalReportJPanel gettRP() {
        return tRP;
    }

    public void settRP(totalReportJPanel tRP) {
        this.tRP = tRP;
    }

    public detailReportMovie getdRM() {
        return dRM;
    }

    public void setdRM(detailReportMovie dRM) {
        this.dRM = dRM;
    }

    public detailReportFood getdRF() {
        return dRF;
    }

    public void setdRF(detailReportFood dRF) {
        this.dRF = dRF;
    }
    ...
    private void createChart() {
        ...
        blankPlotChart.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                if (dRM != null) {
                    fTST.showForm(dDRM);
                    dDRM.initDataDateInMonth(selectedIndex + 1, fTST.getName());
                }
                if (dRF != null) {
                    fTST.showForm(dDRF);
                    dDRF.initDataDateInMonth(selectedIndex + 1, fTST.getName());
                }
            }
        });
    }
    ...
    private void changeSelectedIndex(int index) {
        if (index != -1) {
            ...
            this.mDC = model.get(index);
            if (mRP != null && fRP != null) {
                if (this.index == 0) {
                    mRP.initData(reportDAO.getAmountMovieTopThreeInMonth(mDC.getLabel()));
                } else if (this.index == 1) {
                    fRP.initData(reportDAO.getAmountFoodTopThreeInMonth(mDC.getLabel()));
                } else if (this.index == 2) {
                    tRP.initData(reportDAO.getAmountMovieTopThreeInMonth(mDC.getLabel()), reportDAO.getAmountFoodTopThreeInMonth(mDC.getLabel()),index+1);
                }
            }
        }
    }
    ...
    public void addLegend(String name, Color color1, Color color2) {
        ...
        mticketUtils.setFont(legend, "Kodchasan-Medium.ttf", 12f);
    ...
    private void startChange(int index) {
        if (this.index != index) {
            if (index == 0) {
                getfTST().showForm(getmRP());
                mRP.initData(reportDAO.getAmountMovieTopThreeInMonth(mDC.getLabel()));
            } else if (index == 1) {
                getfTST().showForm(getfRP());
                fRP.initData(reportDAO.getAmountFoodTopThreeInMonth(mDC.getLabel()));
            } else {
                getfTST().showForm(gettRP());
                tRP.initData(reportDAO.getAmountMovieTopThreeInMonth(mDC.getLabel()), reportDAO.getAmountFoodTopThreeInMonth(mDC.getLabel()), index + 1);
            }
            if (animatorChange.isRunning()) {
                animatorChange.stop();
    ...
    public static Movie getmV() {
        return mV;
    }

    public static void setmV(Movie mV) {
        CurveLineChart.mV = mV;
    }
    ...
    public void setBlankPlotChart(BlankPlotChart blankPlotChart) {
        this.blankPlotChart = blankPlotChart;
    }
